import{d,e as _,t as h,i as f,b as v,c as y,f as M,a as w}from"./footer-e6b0f4f3.js";async function x(){const n=[];for(let s=1;s<=5;s+=1){const e=await d(s);n.push(...e)}return n}async function D(t){try{return(await _()).filter(e=>t.includes(e.id)).map((e,o)=>o===0?e.name:e.name.toLowerCase()).join(", ")}catch(n){console.log(n)}}const L="/js-78-BogdanTitsky/assets/no-image-816a65ed.jpg",$=document.querySelector(".js-upcoming");function b(t){return new Date(t).toLocaleString("uk",{day:"numeric",month:"numeric",year:"numeric"})}function k({backdrop_path:t,overview:n,popularity:s,release_date:e,title:o,vote_average:r,vote_count:l},m){const a=t?`https://image.tmdb.org/t/p/original/${t}`:L,i=n||"Sorry! No description.";return`
  ...
  </div>`}function G(t){$.innerHTML=t}const c=document.querySelector("#upcoming-btn");x().then(async t=>{const n=new Date,s=n.getMonth()+1,e=t.filter(i=>{const u=new Date(i.release_date),g=u.getMonth()+1;return u>n&&g===s});if(e.length===0){console.log("No upcoming films this month.");return}const o=Math.floor(Math.random()*e.length),r=e[o].genre_ids,l=await D(r),m=k(e[o],l);G(m),c.style.display="block";function a(){f(e[o].id)?(c.classList.add("remove-btn"),c.textContent="Remove from my library"):(c.classList.remove("remove-btn"),c.textContent="Add to my library")}c.addEventListener("click",i=>{i.preventDefault(),h(e[o]),a()}),a()});const p=document.querySelector(".weelky-trends-list");async function S(){try{const t=await v("week",3);p.innerHTML=await y(t,1),M(p),w(p)}catch(t){console.error(t)}}S();
